...
data_path = (
    "/Users/julianmoreno/Proyectos/birds_on_mars/qtrees-ai/data/data_single_tile/"
)
...
def merge_sunhour_maps(sunhours_folder, target_file):
    for subdir in os.listdir(sunhours_folder):
        ...
        if not os.path.isdir(sunhour_maps_subdir):
            continue

        tiff_files = [
            os.path.join(sunhour_maps_subdir, f)
            for f in os.listdir(sunhour_maps_subdir)
            if f.endswith(".tiff")
        ]
...
def process_all_tiles(tiles_folder, slope_aspect_folder, selected_dates):
    for file_path in glob.glob(os.path.join(tiles_folder, "*.tiff")):
        ...
        if not (os.path.exists(slope_path) and os.path.exists(aspect_path)):
            run_slope_aspect_processing(
                elevation_map=file_path, slope_path=slope_path, aspect_path=aspect_path
            )
        ...
        for day in selected_dates:
            ...
            if not os.path.exists(sunhours_file_path):
                insoltime_calc(
                    elevation_map=file_path,
                    slope_path=slope_path,
                    aspect_path=aspect_path,
                    sunhours_file_path=sunhours_file_path,
                    day=day,
                )


process_all_tiles(
    tiles_folder=elevation_maps_folder,
    slope_aspect_folder=slope_aspect_folder,
